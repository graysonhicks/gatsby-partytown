...
(window => {
        ...
        if (instance) {
            ...
            if (setInc > 99999) {
                instances.forEach(((instance, instanceId) => {
                    instance.nodeType && !instance.isConnected && instances.delete(instanceId);
                }));
                setInc = 0;
            }
        }
    ...
    const serializeForWorker = ($winId$, value, added, type, cstrName) => {
        if (void 0 !== value) {
            ...
            if ("object" === type) {
                return "" === (cstrName = getConstructorName(value)) ? [ 8, {} ] : "Window" === cstrName ? [ 6, {
                    $winId$: $winId$,
                    $instanceId$: 0
    ...
    const readMainPlatform = win => {
        ...
        const $interfaces$ = [ readImplementation("Window", win), readImplementation("Node", textNode) ];
        const config = win.partytown || {};
        const libPath = (config.lib || "/~partytown/") + "debug/";
        const initWebWorkerData = {
            $config$: JSON.stringify(config, ((k, v) => {
                "function" == typeof v && (v = String(v)).startsWith(k + "(") && (v = "function " + v);
                return v;
            })),

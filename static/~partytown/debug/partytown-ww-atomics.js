...
(self => {
    ...
    const addStorageApi = (win, storageName, items) => {
        const getIndexByKey = key => items.findIndex((i => i[STORAGE_KEY] === key));
        const storage = {
            getItem(key) {
            ...
            setItem(key, value) {
                const index = getIndexByKey(key);
                index > -1 ? items[index][STORAGE_VALUE] = value : items.push([ key, value ]);
    ...
    const createNodeInstance = (winId, instanceId, nodeName, namespace) => new (nodeConstructors[nodeName] ? nodeConstructors[nodeName] : nodeName.includes("-") ? nodeConstructors.UNKNOWN : self.HTMLElement)(winId, instanceId, [], nodeName, namespace);
    ...
    const cachedReadonlyProps = (Cstr, propNames) => propNames.split(",").map((propName => definePrototypeProperty(Cstr, propName, {
        get() {
    ...
    const cachedDimensionProps = Cstr => dimensionPropNames.map((propName => definePrototypeProperty(Cstr, propName, {
        get() {
